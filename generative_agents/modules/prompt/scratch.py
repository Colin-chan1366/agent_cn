...
class Scratch:
    ...
    def prompt_schedule_decompose(self, plan, schedule):
        ...
        def _callback(response):
            # 更灵活的匹配模式
            patterns = [
                # 原格式: 1) Aisha *planned* to do X (duration: 15, remaining: 45)
                r"\d{1,2}\) .*\*planned\* (.*)[\(\(]+duration[:: ]+(\d{1,2})[,,\ ]+remaining[:: ]+\d*[\)\)]",
                # 新格式: 1. Activity (duration: 15 minutes, remaining: 45 minutes)
                r"\d{1,2}[\.\)] (.+?) \(duration: (\d{1,2})",
                # 备用格式: 1. Activity (15 min)
                r"\d{1,2}[\.\)] (.+?) \((\d{1,2}) min",
            ]
            schedules = parse_llm_output(response, patterns, mode="match_all", ignore_empty=True)
            if not schedules:
                # 如果都没匹配到，使用 failsafe
                return failsafe
            schedules = [(s[0].strip(".").strip(), int(s[1])) for s in schedules]
    ...
    def prompt_describe_emoji(self, describe):
        ...
        def _callback(response):
            # 正则表达式：匹配大多数emoji
            emoji_pattern = r"([\U0001F600-\U0001F64F]|"   # 表情符号
            emoji_pattern += r"[\U0001F300-\U0001F5FF]|"   # 符号和图标
            emoji_pattern += r"[\U0001F680-\U0001F6FF]|"   # 运输和地图符号
            emoji_pattern += r"[\U0001F700-\U0001F77F]|"   # 午夜符号
            emoji_pattern += r"[\U0001F780-\U0001F7FF]|"   # 英镑符号
            emoji_pattern += r"[\U0001F800-\U0001F8FF]|"   # 合成扩展
            emoji_pattern += r"[\U0001F900-\U0001F9FF]|"   # 补充符号和图标
            emoji_pattern += r"[\U0001FA00-\U0001FA6F]|"   # 补充符号和图标
            emoji_pattern += r"[\U0001FA70-\U0001FAFF]|"   # 补充符号和图标
            emoji_pattern += r"[\U00002702-\U000027B0]+)"  # 杂项符号
    ...
    def prompt_decide_wait(self, agent, other, focus):
        example1 = self.build_prompt(
            "decide_wait_example",
            {
                ...
                "reason": "Reasoning: Jane and Liz both want to use the bathroom. It would be strange for Jane and Liz to use the bathroom at the same time. So, since Liz is already using the bathroom, the best choice for Jane is to wait to use the bathroom.\n",
        ...
        example2 = self.build_prompt(
            "decide_wait_example",
            {
                ...
                "reason": "Reasoning: Sam might have lunch in the restaurant. Sarah might go to the laundry room to do laundry. Since Sam and Sarah need to use different areas, their actions do not conflict. So, since Sam and Sarah will be in different areas, Sam continues to have lunch now.\n",
        ...
        def _status_des(a):
            ...
            if event.address:
                loc = " in {} at {}".format(event.address[-2], event.address[-1])
            if not a.path:
    ...
    def prompt_summarize_chats(self, chats):
        ...
        if len(chats) > 1:
            ...
            failsafe = "{}'s words were not responded to".format(chats[0][0])
