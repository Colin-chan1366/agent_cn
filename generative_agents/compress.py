            ...
            for agent_name, agent_data in agents.items():
                ...
                for i in range(frames_per_step):
                    ...
                    if moving:
                        ...
                        # 针对睡觉和对话设置图标
                        if "sleeping" in action.lower() or "睡觉" in action:
                            action = "😴 " + action
...
def generate_report(checkpoints_folder, compressed_folder, compressed_file):
    ...
    def extract_description():
        markdown_content = "# Basic Personas\n\n"
        for agent_name in personas:
            json_path = f"frontend/static/assets/village/agents/{agent_name}/agent.json"
            with open(json_path, "r", encoding="utf-8") as f:
                json_data = json.load(f)
                markdown_content += f"## {agent_name}\n\n"
                markdown_content += f"Age: {json_data['scratch']['age']}  \n"
                ...
                markdown_content += f"Currently: {json_data['currently']}\n\n"
